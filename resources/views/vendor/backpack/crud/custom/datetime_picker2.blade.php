...
@php
    $r_id = rand(1,9999);
    $f_id = (isset($field['name'])? str_replace(']','',str_replace('[','',$field['name'])) :'my-id').$r_id;
@endphp
<?php
...
<input type="hidden" name="{{ $field['name'] }}" id="{{$f_id}}"
       value="{{ old($field['name']) ? old($field['name']) : (isset($field['value']) ? $field['value'] : (isset($field['default']) ? $field['default'] : '' )) }}">
{{--  <label>{!! $field['label'] !!}</label>--}}
@include('crud::inc.field_translatable_icon')
<div class="input-group date" style="width: 100%">
    <input class="inc xxx-{{$field['name']}}" style="width: 100%" id="xxx_{{$f_id}}"
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields) || isset($field['showOneTime']) )

    @if($field['showOneTime']  == 1)
...
//                $('[data-bs-datetimepicker]').each(function () {
                $('.xxx-{{$field['name']}}').each(function () {
                    ...
                    $picker.on('dp.change', function (e) {
                        ...
                        p.find('[name={{$field['name']}}]').val(sqlDate);
                    });
