...
<div style="margin-bottom: 30px;">
    ...
    <h3 align="center" style="margin-top: -50px;">{{_t('OPEN DETAIL REPORT')}}</h3>
    ...
    <table class="" style="width: 100%">
        ...
        @foreach($rows as $row)
                    ...
                    <table width="100%">
                        <tr>
                            <td width="33%" valign="top">
                                {{_t('Open Number')}}: {{$row->open_number}}<br>
                            </td>
                            <td width="34%" valign="top">
                                {{_t('Open Due')}}: {{\Carbon\Carbon::parse($row->_date_)->format('d/m/Y') }}<br>
                            </td>
                            <td width="33%" valign="top">
            ...
            <table width="100%">
                ...
                <tbody class="border">
                    @php
                        $rowss = \App\Models\OpenItemDetail::where('ref_id',$row->id)->get();
                    @endphp
                    @foreach($rowss as $rd)
                        @php
                                $total_qty+= ($rd->qty);
